...
echo '{"mod_php7":{"enabled":true},"wordpress":{"woocommerce":true}}' > /opt/local/woocommerce.json
/usr/bin/yum install -y jq
/usr/bin/jq -s '.[0] * .[1]' /opt/local/woocommerce.json /opt/local/amimoto.json > /tmp/amimoto.json
...
if [ ! -f /etc/httpd/conf/httpd.conf ]; then
  /usr/bin/chef-solo -o amimoto::httpd_default -c /opt/local/solo.rb -j /opt/local/amimoto.json -l error
  /usr/bin/chef-solo -o amimoto::mod_php7 -c /opt/local/solo.rb -j /opt/local/amimoto.json -l error
  /usr/bin/chef-solo -o amimoto::httpd -c /opt/local/solo.rb -j /opt/local/amimoto.json -l error
fi
