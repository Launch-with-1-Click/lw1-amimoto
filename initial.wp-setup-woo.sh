...
echo '{"hhvm":{"enabled":true},"wordpress":{"woocommerce":true}}' > /opt/local/woocommerce.json
...
if [ ! -d /etc/hhvm ]; then
  /usr/bin/chef-solo -o amimoto::hhvm -c /opt/local/solo.rb -j /opt/local/amimoto.json -l error
fi
